...
public class RezeptRepository {
    ...
    public Optional<Rezept> update(Long id, Rezept rezept) {
        String sql = "UPDATE mocktail SET name = ?, zutaten = ?, zubereitung = ?, preis = ? WHERE id = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, rezept.getName());
            stmt.setString(2, rezept.getZutaten());
            ...
            if (affected > 0) {
                Rezept updated = new Rezept(id, rezept.getName(), rezept.getZutaten(), rezept.getZubereitung(), 0.0);
